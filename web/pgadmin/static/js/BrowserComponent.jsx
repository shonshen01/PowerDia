  ...
  panelExtra: (panelData) => {
    ...
    return (
      <DefaultButton
        size="xs"
        onClick={() => {
          const layoutDocker = pgAdmin.Browser.docker.default_workspace;
          layoutDocker.closeAll(panelData.activeId);
        }}
      >
        {gettext('Close All')}
      </DefaultButton>
...
function Layouts({ browser }) {
  ...
  return (
    <ApplicationStateProvider>
      <div style={{ display: 'flex', height: (browser != 'Electron' ? 'calc(100% - 30px)' : '100%'), backgroundColor: 'inherit' }}>
