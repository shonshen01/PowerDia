...
export default function QueryToolComponent({ params, pgWindow, pgAdmin, selectedNodeInfo, qtPanelDocker, qtPanelId, eventBusObj }) {
  ...
  const hasAutoExecuted = useRef(false);    // PowerDia: guards one-shot macro auto-execution
  const hasViewDataExecuted = useRef(false); // PowerDia: guards one-shot View Data auto-execution
  ...
  // PowerDia: Results-only mode — remove upper editor workspace entirely
  const isResultsOnlyMode = !qtState.params.is_query_tool || Boolean(qtState.params.macro_sql);
  if (isResultsOnlyMode) {
    defaultLayout.dockbox.children = [
      defaultLayout.dockbox.children[1]  // Keep only the lower Results panel
    ];
  }
  ...
  const getSQLScript = () => {
    ...
    } else if (qtState.params.macro_sql) {
      // PowerDia (Plan B): Execution is deferred — fired by useEffect once qtState.connected is true.
      // This guarantees ResultSet.jsx has fully mounted and registered its EXECUTION_START listener.
      setQtStatePartial({ editor_disabled: false });
  ...
  useEffect(()=>{
    ...
    // PowerDia: If ResultsOnly Mode, Query.jsx is gone, so we must bridge the closing event chain manually
    // otherwise the panel will refuse to close!
    // Since MainToolBar is also gone, bypass WARN_TXN_CLOSE and exit directly.
    if (isResultsOnlyMode) {
      eventBus.current.registerListener(QUERY_TOOL_EVENTS.WARN_SAVE_TEXT_CLOSE, ()=>{
        eventBus.current.fireEvent(QUERY_TOOL_EVENTS.FORCE_CLOSE_PANEL);
      });
    }
  ...
  // PowerDia (Plan B): Fire macro SQL exactly once, after the DB connection is confirmed.
  // qtState.connected only becomes true after initializeQueryTool()'s async API call completes.
  // By that point, all child components (including ResultSet.jsx) have mounted and registered
  // their event listeners — so EXECUTION_START is guaranteed to reach a live audience.
  useEffect(() => {
    if (isResultsOnlyMode && qtState.params.macro_sql && qtState.connected && !hasAutoExecuted.current) {
      hasAutoExecuted.current = true;
      eventBus.current.fireEvent(QUERY_TOOL_EVENTS.EXECUTION_START, qtState.params.macro_sql, {
        external: true,
        macroSQL: qtState.params.macro_sql,
      });
    }
  }, [qtState.connected]);

  // PowerDia (Plan B for View Data / All Rows):
  // When is_query_tool=false, Query.jsx is not mounted (Results-Only mode strips the upper pane).
  // The normal chain TRIGGER_EXECUTION → Query.jsx → EXECUTION_START is broken.
  // We bridge directly: fire EXECUTION_START with an empty query string.
  // postExecutionApi() ignores the query when isQueryTool=false and hits the
  // view_data_start endpoint instead, which generates the SELECT * on the backend.
  useEffect(() => {
    if (!qtState.params.is_query_tool && !qtState.params.macro_sql
        && qtState.connected && !hasViewDataExecuted.current) {
      hasViewDataExecuted.current = true;
      eventBus.current.fireEvent(QUERY_TOOL_EVENTS.EXECUTION_START, '', {
        external: false,
      });
    }
  }, [qtState.connected]);
      ...
      <QueryToolConnectionContext.Provider value={queryToolConnContextValue}>
        <QueryToolEventsContext.Provider value={eventBus.current}>
          <Box width="100%" height="100%" display="flex" flexDirection="column" flexGrow="1" tabIndex="0" ref={containerRef}>
            {!isResultsOnlyMode && <ConnectionBar
              connected={qtState.connected}
            ...
            />}
            {!isResultsOnlyMode && React.useMemo(() => (
              <MainToolBar
                containerRef={containerRef}
