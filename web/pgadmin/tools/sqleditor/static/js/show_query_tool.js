...
export function showQueryTool(queryToolMod, pgBrowser, url, treeIdentifier, transId, sqlId, is_new_tab, macroSQL) {
  ...
  const gridUrl = generateUrl(transId, parentData, sqlId);
  const launchParams = { query_url: url };
  if (macroSQL) {
    launchParams.macro_sql = macroSQL;
  }
  launchQueryTool(queryToolMod, transId, gridUrl, queryToolTitle, launchParams);
}
