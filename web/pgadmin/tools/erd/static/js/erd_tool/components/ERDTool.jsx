...
import NetworkConfigDialog from '../dialogs/NetworkConfigDialog';
...
export default class ERDTool extends React.Component {
  ...
  registerEvents() {
    ...
    // PowerDia: open the config dialog when toolbar fires LOAD_NETWORK
    this.eventBus.registerListener(ERD_EVENTS.LOAD_NETWORK, () => {
      this.setState({ network_config_open: true });
    });
  }
  ...
  // PowerDia ────────────────────────────────────────────────────────────────────
  async loadNetworkData(config) {
    this.setState({ network_config_open: false });
    this.setLoading(gettext('Loading network hierarchy...'));
    try {
      const res = await this.apiObj.get(
        url_for('erd.network_hierarchy', {
          trans_id: this.props.params.trans_id,
          sgid: this.props.params.sgid,
          sid:  this.props.params.sid,
          did:  this.props.params.did,
        }),
        { params: { table: config.table, level1: config.level1, level2: config.level2, level3: config.level3 } }
      );

      // Bug Fix #2: this.diagram.initializeModel() — not this.initializeModel()
      this.diagram.initializeModel();
      this.registerModelEvents();

      const { substations } = res.data.data;
      const COL_GAP  = 300;  // horizontal gap: substation → feeder → transformer
      const ROW_GAP  = 80;   // extra vertical gap between substation groups
      const LEAF_GAP = 160;  // vertical gap between transformer (leaf) nodes
      const START_X  = 50;   // fixed left-edge for all substations
      let currentY   = 50;   // running Y — increases as we stack substations

      substations.forEach((sub) => {
        // All substations share the same X column → vertical stacking
        const subNode = this.diagram.addPowerNode('substation', sub.level1, [START_X, currentY]);

        let feederY = currentY;
        sub.feeders.forEach((feeder) => {
          const feederNode = this.diagram.addPowerNode(
            'feeder', feeder.level2, [START_X + COL_GAP, feederY]
          );
          this.diagram.addSimpleLink(subNode, feederNode);

          feeder.leaves.forEach((leaf, li) => {
            const leafNode = this.diagram.addPowerNode(
              'transformer', leaf, [START_X + COL_GAP * 2, feederY + li * LEAF_GAP]
            );
            this.diagram.addSimpleLink(feederNode, leafNode);
          });
          feederY += feeder.leaves.length * LEAF_GAP + ROW_GAP;
        });

        // Advance Y past this substation's full height before drawing the next one
        currentY = feederY + ROW_GAP;
      });
    ...
    return (
      <StyledBox ref={this.containerRef} height="100%" display="flex" flexDirection="column">
        ...
        <div className='ERDTool-diagramContainer' data-test="diagram-container" ref={this.diagramContainerRef} onDrop={this.onDropNode} onDragOver={e => {e.preventDefault();}} tabIndex={0}>
          ...
          {/* PowerDia: Network Config Dialog */}
          {this.state.network_config_open && (
            <NetworkConfigDialog
              transId={this.props.params.trans_id}
              sgid={this.props.params.sgid}
              sid={this.props.params.sid}
              did={this.props.params.did}
              api={this.apiObj}
              onLoad={(config) => this.loadNetworkData(config)}
              onClose={() => this.setState({ network_config_open: false })}
            />
          )}
