...
import { SubstationNodeFactory, FeederNodeFactory, TransformerNodeFactory } from './nodes/PowerDiaNodes';
...
export default class ERDCore {
  ...
  initializeEngine() {
    ...
    // PowerDia: register hierarchy node types (additive — does not replace existing factories)
    this.engine.getNodeFactories().registerFactory(new SubstationNodeFactory());
    this.engine.getNodeFactories().registerFactory(new FeederNodeFactory());
    this.engine.getNodeFactories().registerFactory(new TransformerNodeFactory());
  ...
  optimizePortsPosition(node) {
    /* Only TableNodeModel nodes have getLinks() and getPortAttnum() —
       PowerDia hierarchy nodes (Substation / Feeder / Transformer) must
       be skipped, otherwise we get "node.getLinks is not a function". */
    if (!(node instanceof TableNodeModel)) {
      return;
    }
    Object.values(node.getLinks()).forEach((link)=>{
      ...
      /* Guard: both endpoints must also be TableNodeModel nodes */
      if (!(sourcePort?.getNode() instanceof TableNodeModel) ||
          !(targetPort?.getNode() instanceof TableNodeModel)) {
        return;
      }
  ...
  // PowerDia ─── create a hierarchy node (substation / feeder / transformer) ──
  addPowerNode(type, label, position = [50, 50]) {
    const factory = this.getEngine().getNodeFactories().getFactory(type);
    const node = factory.generateModel({ initialConfig: { powerDiaLabel: label } });
    node.setPosition(position[0], position[1]);
    this.getModel().addNode(node);
    return node;
  }

  // PowerDia ─── draw a simple arrow between two PowerDia nodes ──────────────
  addSimpleLink(sourceNode, targetNode) {
    const sourcePort = Object.values(sourceNode.getPorts()).find(p => p.getName() === 'right');
    const targetPort = Object.values(targetNode.getPorts()).find(p => p.getName() === 'left');
    if (sourcePort && targetPort) {
      const link = sourcePort.createLinkModel() || new OneToManyLinkModel();
      link.setSourcePort(sourcePort);
      link.setTargetPort(targetPort);
      this.getModel().addLink(link);
      return link;
    }
    return null;
  }
  ...
  getNodesData() {
    return this.getEngine().getModel().getNodes()
      .filter((node) => typeof node.getData === 'function')
  ...
  async dagreDistributeNodes() {
    ...
    // Switch left/right ports — only for table nodes (PowerDia nodes are skipped inside).
