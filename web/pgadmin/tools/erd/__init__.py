...
class ERDModule(PgAdminModule):
    ...
    def get_exposed_url_endpoints(self):
        ...
        return [
            ...
            'erd.prequisite',
            'erd.close',
            'erd.network_get_tables',
            'erd.network_get_columns',
            'erd.network_hierarchy',
        ]
...
# ─── PowerDia: Network Hierarchy Endpoints ────────────────────────────────────

# ─── PowerDia: Network Hierarchy Endpoints ────────────────────────────────────


@blueprint.route(
    '/network_get_tables/<int:trans_id>/<int:sgid>/<int:sid>/<int:did>',
    methods=['GET'],
    endpoint='network_get_tables'
)
@pga_login_required
def network_get_tables(trans_id, sgid, sid, did):
    """Return all user-accessible tables as schema.table strings."""
    conn = _get_connection(sid, did, trans_id)
    sql = """
    ...
    return make_json_response(data=[row['full_name'] for row in res['rows']])


@blueprint.route(
    '/network_get_columns/<int:trans_id>/<int:sgid>/<int:sid>/<int:did>',
    methods=['GET'],
    endpoint='network_get_columns'
)
@pga_login_required
def network_get_columns(trans_id, sgid, sid, did):
    """Return column names for the given schema.table."""
    full_table = request.args.get('table', '')
    if '.' not in full_table:
        return internal_server_error(errormsg='Invalid table — expected schema.table')
    ...
    """
    status, res = conn.execute_dict(sql, {'schema': schema, 'table': table})
    if not status:
        return internal_server_error(errormsg=res)
    return make_json_response(data=[row['column_name'] for row in res['rows']])


@blueprint.route(
    '/network_hierarchy/<int:trans_id>/<int:sgid>/<int:sid>/<int:did>',
    methods=['GET'],
    endpoint='network_hierarchy'
)
@pga_login_required
def network_hierarchy(trans_id, sgid, sid, did):
    """Build the 3-level hierarchy from a user-selected table + column mapping."""
    args       = request.args
    full_table = args.get('table', '')
    level1     = args.get('level1', '')
    level2     = args.get('level2', '')
    level3     = args.get('level3', '')

    if not all([full_table, level1, level2, level3]):
        return internal_server_error(
            errormsg='Missing required params: table, level1, level2, level3')
    if '.' not in full_table:
        return internal_server_error(errormsg='Invalid table — expected schema.table')
    ...
    sql = f"""
        SELECT
            {qi(level1)} AS level1,
            {qi(level2)} AS level2,
            array_agg(DISTINCT {qi(level3)} ORDER BY {qi(level3)}) AS leaves
        FROM {qi(schema)}.{qi(table)}
        WHERE {qi(level1)} IS NOT NULL
          AND {qi(level2)} IS NOT NULL
          AND {qi(level3)} IS NOT NULL
        GROUP BY {qi(level1)}, {qi(level2)}
        ORDER BY {qi(level1)}, {qi(level2)}
    """
    status, res = conn.execute_dict(sql)
    if not status:
        return internal_server_error(errormsg=res)
    ...
    for row in res['rows']:
        sub  = row['level1']
        feed = row['level2']
        leaves = row['leaves'] if isinstance(row['leaves'], list) else [row['leaves']]
        subs.setdefault(sub, {})[feed] = leaves

    result = [
        {
            'level1': sub,
            'feeders': [
                {'level2': feed, 'leaves': leaves}
                for feed, leaves in feeders.items()
            ]
        }
        for sub, feeders in subs.items()
    ]
    return make_json_response(data={'substations': result})
