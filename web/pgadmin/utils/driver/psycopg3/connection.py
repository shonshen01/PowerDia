    ...
    def connect(self, **kwargs):
        ...
        try:
            ...
            with ConnectionLocker(manager.kerberos_conn):
                ...
                # -----------------------------------------------------------------
                # PowerDia: PgBouncer Connection Swap (Phase 2)
                #
                # When PGBOUNCER_ENABLED is True and no SSH tunnel is active,
                # intercept the DSN and redirect to the appropriate PgBouncer pool:
                #
                #   conn_id starts with 'CONN:' → Query Tool / Debugger
                #       → Session Mode pool (port PGBOUNCER_SESSION_PORT)
                #         Pins the real connection for the full user session so
                #         multi-statement transactions (BEGIN … COMMIT) are safe.
                #
                #   anything else (e.g. 'DB:') → Dashboard / tree / Macros
                #       → Transaction Mode pool (port PGBOUNCER_TRANSACTION_PORT)
                #         Releases the real connection after every query for
                #         maximum efficiency across 32+ Gunicorn workers.
                #
                # SSH tunnel connections are excluded: the tunnel already provides
                # a local endpoint and re-routing it through PgBouncer would fail.
                # -----------------------------------------------------------------
