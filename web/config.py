...
# This enables high-concurrency, multi-worker (Gunicorn) deployments by
# solving the connection affinity problem across WSGI worker processes.
#
# Two pools are required — configure PgBouncer with two separate listeners:
#   Pool 1 — Transaction Mode (port 6432):
#       Stateless tools: Dashboards, tree navigation, Macros.
#       Connections released after every query for maximum efficiency.
#
#   Pool 2 — Session Mode (port 6433):
#       Stateful tools: Query Tool, Debugger.
#       Connections pinned per session to preserve BEGIN/COMMIT integrity.
#
# Leave PGBOUNCER_ENABLED = False for local desktop/development use.
